package 
{
	...
	public class milestone1_2 extends MovieClip
	{
		var initXmlData:XML;
		var newXmlData:XML;
...
//			trace(localData.data.cityName);
		...
		function textHandler(event:KeyboardEvent)
		{
		   ...
		   if(event.charCode == 13)
		   {
			   clearStage();
			   var myLoader:URLLoader = new URLLoader();
			   myLoader.load(new URLRequest("http://api.openweathermap.org/data/2.5/forecast/daily?q=" + cityInputText.text + "&mode=xml&units=metric&cnt=7"));
